...
REM Purpose: Check all files in a directory via copybadfile.py, and see 
REM whether the files can be read out normally.
...
if not defined param1 (
	call :Echos Error: Need a directory path as parameter.
	exit /b 4
)

set dircheck=%param1%

if not exist "%dircheck%" (
	call :Echos The input directory does not exist: "%dircheck%"
	exit /b
)

REM ======== Now start our work ========
REM Recursively find each file in dircheck and call copybadfile.py .

set count=0
for /R "%dircheck%" %%a in (*.*) do (
	set /A count=count+1
)
...
for /R "%dircheck%" %%a in (*.*) do (
	set /A count=count+1
	echo.
	call :Echos ======== [#!count!/!total!] %%a
	call :EchoAndExec python "%batdir%\copybadfile.py" "%%a" -
	if not !errorlevel!==0 (
		if !errorlevel!==74 (
	...
	call :Echos Bad files encountered: !badfiles!
)
